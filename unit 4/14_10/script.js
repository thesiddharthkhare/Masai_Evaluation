...
let cart = [];
let filteredProucts = [...products];

function displayProducts(productsToDisplay) {
   productList.innerHTML = ``;
   productsToDisplay.forEach(product => {
        const avgRating = calculateAvgRating(product.id);
        productList.innerHTML += `
            <div class="product-card">
                <div class="image-slider">
                    <img src="${product.images[0]}" alt="${product.title}">
                    <button onclick="nextImage(${product.id})">Next</button>
                    <button onclick="prevImage(${product.id})">Prev</button>
                </div>
...
function calculateAvgRating(productId) {
    const productReviews = reviews.filter(reviews => reviews.productId === productId);
...
filterBtn.addEventListener('click', () => {
    ...
    displayProducts(filteredProucts);
});


searchBar.addEventListener('input', () => {
    const searchTerm = searchBar.value.toLowerCase();
    filteredProucts = products.filter(product => product.title.toLowerCase().includes(searchTerm));
    displayProducts(filteredProucts);
});
